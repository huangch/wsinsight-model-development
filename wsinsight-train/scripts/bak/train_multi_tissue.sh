#!/usr/bin/env bash
# Full-cycle pooled training over one or more tissues: annotate (kurtorank) ->
# segment -> transfer -> tile -> split -> train -> validate -> export.
# Includes cellpose OOM guards and auto-tune. Trained head + report land under
# models/<tissue>/, where <tissue> is the comma-joined scope you pass.
#
