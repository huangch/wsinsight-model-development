"""Entry point for the ``kurtorank-mcp`` console script.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="kurtorank-mcp",
        description="Run the Kurtorank MCP server (Model Context Protocol).",
    )
    parser.add_argument(
        "--http",
        metavar="HOST:PORT",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Max concurrent background jobs (default: 2). Census downloads "
        "are network-bound, so serialising too aggressively slows things down.",
    )
    parser.add_argument(
        "--cwd",
        default=None,
        help="Working directory for child kurtorank invocations (default: inherit).",
    )
    parser.add_argument(
        "--sync",
        action="store_true",
        help="Run every kurtorank invocation synchronously (disable job manager).",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
    ...
    log = logging.getLogger("kurtorank.mcp")

    from kurtorank.mcp.server import build_server
