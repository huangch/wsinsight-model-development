#!/usr/bin/env bash
# Example 2 of 3 -- SINGLE-TISSUE training with a SLIDE-LEVEL split.
#
# Slide-level counterpart of train_one_tissue_by_tile.sh. --by-slide holds out
# whole slides, so no tile from a validation slide is ever seen in training and
# the reported scores are free of within-slide leakage. This is the split a
# reviewer means by "held out at the slide level".
#
# How many slides are held out (wsitrain/splits.py::split_tiles):
#   n_val = max(1, min(round(n_slides * VAL_FRAC), n_slides - 1))
# applied per tissue, so with VAL_FRAC=0.20:
#   breast      8 slides -> round(1.6) = 2   -> 6 train / 2 val
...
#        VAL_FRAC=0.125 bash scripts/train_one_tissue_by_slide.sh breast
#
...
if [ "$#" -lt 1 ]; then
  echo "Usage: bash scripts/train_one_tissue_by_slide.sh <tissue> [input_dir] [output_dir]" >&2
  exit 2
fi
...
OUT="${3:-$ROOT/models/${TISSUE}_by_slide}"          # own scope: manifest is per output+tissue
TASK="${TASK:-pantissue}"
