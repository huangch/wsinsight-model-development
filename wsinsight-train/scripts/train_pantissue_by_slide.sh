#!/usr/bin/env bash
# Example 1 of 3 -- PAN-TISSUE training with a SLIDE-LEVEL split.
#
# Slide-level counterpart of train_pantissue_by_tile.sh. --tissue pantissue
# pools every sample discovered under the input tree into a single head, and
# --by-slide holds out whole slides so no tile from a validation slide is ever
# seen in training.
#
# The holdout is stratified per tissue (wsitrain/splits.py::split_tiles): each
# tissue contributes max(1, min(round(n_slides * VAL_FRAC), n_slides - 1))
# validation slides, and a tissue with a single slide is skipped entirely
# rather than becoming an unscoreable leave-tissue-out case. That makes this
...
#              nucleus is looked up for each Xenium cell). Under the
#              default xenium-coords no segmenter is needed, so
#              --segmenter is not passed at all; SEGMENTER is consulted
#              only when NUCLEI_SOURCE=he-mask is set explicitly.
...
echo "== pantissue, slide-level split (task=$TASK, nuclei_source=$NUCLEI_SOURCE, segmenter=$HEADLINE_SEGMENTER, transform=$TRANSFORM, val_frac=$VAL_FRAC, architecture=$ARCHITECTURE) =="
