"""Background-job tracking for long-running ``kurtorank`` invocations.

Each long-running call spawns ``kurtorank <subcmd> …`` in a child
process and returns a ``job_id``. The agent polls ``job_status`` /
``job_logs`` / ``cancel_job`` to monitor progress, mirroring the
sibling ``wsitrain.mcp.jobs`` module so all the workspace's MCP
servers expose the same job-control surface.
"""
...
class JobManager:
    ...
    def __init__(self, max_concurrent: int | None = None) -> None:
        ...
        self.max_concurrent = (
            ...
            else self._default_concurrency()
        )
        self._sem = threading.BoundedSemaphore(self.max_concurrent)

    def _default_concurrency(self) -> int:
        # Census downloads are network-bound; CPU is also fine. Default 2.
        return 2

    def submit(self, argv: list[str], cwd: str | None = None) -> Job:
        """Spawn ``kurtorank <subcmd> …`` and return a tracked job."""
        job_id = uuid.uuid4().hex
        cmd = [sys.executable, "-m", "kurtorank", *argv]
        ...
        threading.Thread(
            target=self._supervise, args=(job,), daemon=True, name=f"kurtorank-job-{job_id[:6]}"
        ).start()
        return job
