...
def _wipe_dir(p: Path, pattern: str = "*") -> int:
    """Best-effort glob-delete; return number of files removed."""
...
def _wipe_segment(out, cfg) -> int:
    """``masks/<tissue>/*.npy`` + the recipe sidecar."""
    from . import paths as _paths
    n = _wipe_dir(paths.masks_dir(out, cfg.tissue), "*.npy")
    p = paths.masks_dir(out, cfg.tissue).parent / f"{paths.masks_dir(out, cfg.tissue).name}.recipe.json"
    if p.is_file():
        p.unlink()
        n += 1
    return n
...
def _wipe_transfer(out, cfg) -> int:
    """``nuclei/<tissue>/`` (label_map lives under ``trainingset/``)."""
    from . import paths as _paths
    return _wipe_dir(paths.nuclei_dir(out, cfg.tissue))


def _wipe_tile(out, cfg) -> int:
    """``trainingset/<tissue>/train/{images,labels}``, ``cache/``, and ``stain/``."""
    from . import paths as _paths
    n = 0
    n += _wipe_dir(paths.images_dir(out, cfg.tissue), "*.png")
    n += _wipe_dir(paths.labels_dir(out, cfg.tissue), "*.csv")
    n += _wipe_dir(paths.tissue_root(out, cfg.tissue) / "cache", "*.h5")
    n += _wipe_dir(out / "stain", "*.npz")
    return n


def _wipe_crop(out, cfg) -> int:
    from . import paths as _paths
    return _wipe_dir(paths.cells_dir(out, cfg.tissue), "*.h5")


def _wipe_split(out, cfg) -> int:
    from . import paths as _paths
    n = 0
    n += _wipe_dir(paths.splits_dir(out, cfg.tissue, cfg.fold))
    # train_configs/<backbone>/<fold>.yaml is regenerated by configrender
    # alongside the next split; stale content would otherwise mislead train.
    n += _wipe_dir(paths.train_config_path(out, cfg.tissue, cfg.backbone, cfg.fold).parent,
                   "*.yaml")
    return n


def _wipe_train(out, cfg) -> int:
    """Drop the extracted-cell cache + the most recent run dir.

    train_configs/ belongs to split, which renders it; deleting it here left
    split marked done with its output gone. Older run dirs under
    ``logs/<tissue>/`` are kept for diagnosis.
    """
    from . import paths as _paths
    n = 0
    n += _wipe_dir(paths.tissue_root(out, cfg.tissue) / "cache", "*.h5")
    # Most recent run-tagged directory only; sort lexicographically under the
    # ISO-8601 timestamp convention (YYYY-MM-DDTHHMMSS_*) so the latest is
    # last.
    log_root = paths.logs_dir(out, cfg.tissue)
    if log_root.is_dir():
        runs = sorted(p for p in log_root.iterdir() if p.is_dir())
        if runs:
            import shutil as _shutil
            _shutil.rmtree(runs[-1])
            n += 1
    return n


def _wipe_validate(out, cfg) -> int:
    from . import paths as _paths
    n = 0
    n += _wipe_dir(paths.report_dir(out, cfg.tissue), "scores.json")
    n += _wipe_dir(paths.report_dir(out, cfg.tissue), "confusion_matrix.png")
    return n


def _wipe_export(out, cfg) -> int:
    from . import paths as _paths
    return _wipe_dir(paths.models_dir(out, cfg.tissue) / "main")


def _wipe_report(out, cfg) -> int:
    from . import paths as _paths
    return _wipe_dir(paths.report_dir(out, cfg.tissue), "summary.txt")


# Stage -> wipe function. Used by both ``--redo-<stage>`` (wipe just that
# stage) and the historical ``--force`` legacy semantics: ``--force`` wipes
# *every* stage so the user can re-run from scratch without hunting for
# specific flags.
_STAGE_WIPES: dict[str, "callable"] = {
    "segment":  _wipe_segment,
    "transfer": _wipe_transfer,
    "tile":     _wipe_tile,
    "crop":     _wipe_crop,
    "split":    _wipe_split,
    "train":    _wipe_train,
    "validate": _wipe_validate,
    "export":   _wipe_export,
    "report":   _wipe_report,
}
...
def reset_cache(stage: str, cfg, out: Path) -> None:
    """Drop the per-stage artefacts so a re-run produces fresh output.

    The historical contract was: --force wipes segment only and is a no-op
    elsewhere. The new contract is the same, but every stage now has a
    per-stage wipe so future ``--redo-<stage>`` flags have a place to land.
    """
    fn = _STAGE_WIPES.get(stage)
    if fn is None:
        return
    n = fn(out, cfg)
    if n:
        import sys as _sys
        _sys.stderr.write(f"[wsitrain] reset_cache({stage}): wiped {n} artefact(s)\n")
...
def _tile_one_slide(cfg, s, *, out, nuc_dir, img_dir, lab_dir, stain_dir, w_target,
    ...
    try:
        with SlideReader(s.he) as reader:
            ...
            rows = range(0, h - cfg.tile_px + 1, stride)
            if progress:
                # A slide is ~11 min, so without this the outer bar looks hung.
                from tqdm import tqdm as _tqdm
                rows = _tqdm(list(rows), desc=f"  {s.sample_id[:38]}", unit="row",
                             ascii=" =", dynamic_ncols=True, leave=False,
                             position=1)
            for ti, y0 in enumerate(rows):
...
def _check_rendered_gpu(cfg_path: Path) -> None:
    """Reject a config the split stage rendered into an unusable state.

    CellViT builds ``f"cuda:{run_conf['gpu']}"`` with no DataParallel path, so
    a comma-separated value reaches torch as the invalid device "cuda:0,1"; an
    empty ``cellvit_path`` (split ran without $CELLVIT_ROOT) reaches it as
    ``torch.load(None)``.
    """
    fields = {}
    for line in cfg_path.read_text().splitlines():
        for key in ("gpu:", "cellvit_path:"):
            if line.startswith(key):
                fields[key.rstrip(":")] = line.split(":", 1)[1].strip()
    gpu = fields.get("gpu", "")
    if "," in gpu:
        raise RuntimeError(
            f"{cfg_path} sets 'gpu: {gpu}', which reaches CellViT as the "
            f'invalid device "cuda:{gpu}". It was rendered by an older '
            "wsitrain; re-render it by re-running with --redo-split.")
    if "cellvit_path" in fields and not fields["cellvit_path"]:
        raise RuntimeError(
            f"{cfg_path} names no CellViT checkpoint: it was rendered while "
            "$CELLVIT_ROOT was unset. Re-render it by re-running with "
            "--redo-split from a shell where CELLVIT_ROOT is exported.")
